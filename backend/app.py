...
def create_app(
    ...
    app.config.from_mapping(
        ...
        SAVE_RESULTS_TO_FIRESTORE=env_bool(
        ...
        HF_PREDICT_URL=os.getenv(
            "HF_PREDICT_URL",
            "",
        ).strip(),
        HF_REQUEST_TIMEOUT_SECONDS=env_int(
    ...
    if inference_provider not in {
        ...
        raise RuntimeError(
            "INFERENCE_PROVIDER must be 'mock' or 'huggingface'."
        )

    if (
        inference_provider == "huggingface"
        and not app.config["HF_PREDICT_URL"]
    ):
        raise RuntimeError(
    ...
    logger.info(
        "Configured inference provider: %s",
        inference_provider,
    )

    if (
        app.config["REQUIRE_AUTH"]
        or app.config["SAVE_RESULTS_TO_FIRESTORE"]
    ):
        ...
        if not project_id:
            raise RuntimeError(
                ...
                "must be set when Firebase authentication or "
                "Firestore saving is enabled."
            )
    ...
    def authenticate_request():
        ...
        try:
            ...
            logger.exception(
                "Unexpected authentication failure: %s",
                exc,
            )
            ...
            jsonify(
                {
                    ...
                    "message": (
                        "The uploaded image must not exceed 10 MB."
                    ),
                }
            ),
            413,
        ...
        return jsonify(
            {
                ...
                "modelLoaded": (
                    ...
                    and bool(app.config["HF_PREDICT_URL"])
                ),
    ...
    def predict():
                ...
                jsonify(
                    {
                        "error": "missing_file",
                        "message": (
                            "Send the image using multipart "
                            "field name 'file'."
                        ),
                    }
                ),
        ...
        if (
            ...
            try:
                result = predict_with_huggingface(
                    ...
                    image_width=width,
                    image_height=height,
                    predict_url=app.config[
                        "HF_PREDICT_URL"
                    ],
                    timeout_seconds=app.config[
                        "HF_REQUEST_TIMEOUT_SECONDS"
                    ],
                    model_version=app.config[
                        "MODEL_VERSION"
                    ],
                )
        ...
        if app.config["SAVE_RESULTS_TO_FIRESTORE"]:
            try:
                ...
                logger.exception(
                    "Could not save the inspection: %s",
                    exc,
                )
